...
def write_results(filename: str, insts: List[Instance]):
    ...
    for inst in insts:
        ...
        for i in range(len(inst.words)):
            ...
            if prediction[i] == 'B-LOC' or prediction[i] == 'I-LOC' or prediction[i] == 'E-LOC':
                word = words[i].replace('"','')
                word = word.replace(",","")
                pred_str += word
                pred_str += ' '
            elif prediction[i] == 'B-ABBR' or prediction[i] == 'I-ABBR' or prediction[i] == 'E-ABBR':
                word = words[i].replace('"','')
                word = word.replace(",","")
                pred_str += filter(words_list, word)
                pred_str += ' '
