...
def parse_arguments(parser):
    ...
    parser.add_argument('--dataset', type=str, default="POI_ext")
    parser.add_argument('--embedding_file', type=str, default="data/embeddings.txt",
                        help="we will be using random embeddings if file do not exist")
    parser.add_argument('--embedding_dim', type=int, default=100)
    parser.add_argument('--optimizer', type=str, default="sgd", help="This would be useless if working with transformers package")
    ...
    parser.add_argument('--model_folder', type=str, default="POI_ext", help="The name to save the model files")
...
def train_model(config: Config, epoch: int, train_loader: DataLoader, dev_loader: DataLoader, test_loader: DataLoader):
    ...
    for i in tqdm(range(1, epoch + 1), desc="Epoch"):
        ...
        for iter, batch in enumerate(train_loader, 1):
            ...
            model.zero_grad()
            if iter%1000 == 0:
                print("--training batch: %d-%d" % (iter,len(train_loader)))
        end_time = time.time()
